...
void example_ckks_basics()
{
    ...
    */
    ...
    /*
    We create plaintexts for 5.0, 3.2, and 2 using an overload of CKKSEncoder::encode
    that encodes the given floating-point value to every slot in the vector.
    */
    Plaintext plain_coeff2, plain_coeff1, plain_coeff0, plain_coeff3;
    encoder.encode(5.0, scale, plain_coeff2);
    encoder.encode(3.2, scale, plain_coeff1);
    encoder.encode(2.0, scale, plain_coeff0);
    encoder.encode(1.0, scale, plain_coeff3);
    ...
    /*
    We need to multiply the x^2 term with 5.0 (plain_coeff2), we instead compute 5.0*x
    first and multiply that with x * 1.0 (plain_coeff3)  to obtain 5.0*x^2. To this end, we compute
    1.0*x and rescale it back from scale 2^80 to something close to 2^40.
    */
    print_line(__LINE__);
    cout << "Compute and rescale 1.0*x." << endl;
    Ciphertext x0_encrypted_coeff3;
    evaluator.multiply_plain(x1_encrypted, plain_coeff3, x0_encrypted_coeff3);
    cout << "    + Scale of 1.0*x before rescale: " << log2(x0_encrypted_coeff3.scale()) << " bits" << endl;
    evaluator.rescale_to_next_inplace(x0_encrypted_coeff3);
    cout << "    + Scale of 1.0*x after rescale: " << log2(x0_encrypted_coeff3.scale()) << " bits" << endl;

    /*
    We need to multiply the x^2 term with 5.0 (plain_coeff2), we instead compute 5.0*x
    first and multiply that with x * 1.0  to obtain 5.0*x^2. To this end, we compute
    5.0*x and rescale it back from scale 2^80 to something close to 2^40.
    */
    print_line(__LINE__);
    cout << "Compute and rescale 5.0*x." << endl;
    Ciphertext x1_encrypted_coeff2;
    evaluator.multiply_plain(x1_encrypted, plain_coeff2, x1_encrypted_coeff2);
    cout << "    + Scale of 5.0*x before rescale: " << log2(x1_encrypted_coeff2.scale()) << " bits" << endl;
    evaluator.rescale_to_next_inplace(x1_encrypted_coeff2);
    cout << "    + Scale of 5.0*x after rescale: " << log2(x1_encrypted_coeff2.scale()) << " bits" << endl;

    /*
    Since x0_encrypted_coeff3 and x1_encrypted_coeff2 have the same exact scale
    the same encryption parameters, we can multiply them together. We write the
    result to x2_encrypted, relinearize, and rescale. Note that again the scale
    is something close to 2^40, but not exactly 2^40 due to yet another scaling
    by a prime. We are down to the last level in the modulus switching chain.
    */
    print_line(__LINE__);
    cout << "Compute, relinearize, and rescale (5.0*x)*x." << endl;
    //evaluator.multiply_inplace(x1_encrypted, x1_encrypted_coeff2);
    evaluator.multiply_inplace(x1_encrypted_coeff2,x0_encrypted_coeff3);
    evaluator.relinearize_inplace(x1_encrypted_coeff2, relin_keys);
    cout << "    + Scale of 5.0*x^2 before rescale: " << log2(x1_encrypted_coeff2.scale()) << " bits" << endl;
    evaluator.rescale_to_next_inplace(x1_encrypted_coeff2);
    cout << "    + Scale of 5.0*x^2 after rescale: " << log2(x1_encrypted_coeff2.scale()) << " bits" << endl;
    ...
    */
    ...
    cout << "    + Modulus chain index for x1_encrypted_coeff2: "
         << context.get_context_data(x1_encrypted_coeff2.parms_id())->chain_index() << endl;
    ...
    above the scales in ciphertexts are:
        ...
        - Product 5.0*x has scale 2^80 and is at level 2;
        - Product 1.0*x has scale 2^80 and is at level 2;
        - We rescaled both down to scale 2^80/P_2 and level 1;
        - Product 5.0*x^2 has scale (2^80/P_2)^2;
        - We rescaled it down to scale (2^80/P_2)^2/P_1 and level 0;
        - Product 3.2*x has scale 2^80;
    ...
    */
    ...
    cout << "    + Exact scale in 5.0*x^2: " << x1_encrypted_coeff2.scale() << endl;
    cout << "    + Exact scale in  3.2*x: " << x1_encrypted.scale() << endl;
    cout << "    + Exact scale in      2: " << plain_coeff0.scale() << endl;
    cout << endl;
    ...
    scale the value of 5.0*x^2 by 2^120/(P_2^2*P_1), which is very close to 1.
    This should not result in any noticeable error.

    Another option would be to encode 2 with scale 2^80/P_2, do a multiply_plain
    with 3.2*x, and finally rescale. In this case we would need to additionally
    make sure to encode 2 with appropriate encryption parameters (parms_id).

    In this example we will use the first (simplest) approach and simply change
    the scale of 5.0*x^2 and 3.2*x to 2^40.
    */
    print_line(__LINE__);
    cout << "Normalize scales to 2^40." << endl;
    x1_encrypted_coeff2.scale() = pow(2.0, 40);
    ...
    */
    ...
    parms_id_type last_parms_id = x1_encrypted_coeff2.parms_id();
    ...
    */
    print_line(__LINE__);
    cout << "Compute 5.0*x^2 + 3.2*x + 2." << endl;
    Ciphertext encrypted_result;
    evaluator.add(x1_encrypted_coeff2, x1_encrypted, encrypted_result);
    ...
    */
    ...
    print_line(__LINE__);
    cout << "Decrypt and decode 5.0*x^2 + 3.2x + 2." << endl;
    cout << "    + Expected result:" << endl;
    vector<double> true_result;
    for (size_t i = 0; i < input.size(); i++)
    {
        double x = input[i];
        true_result.push_back((5.0 * x + 3.2) * x + 2);
    }
